    ...
    for counter, entries in enumerate(classes):
        ...
        for item in entries:
            ...
            if type == "clause":
                ...
                if agb.clauseIsLabeled == True:
                    true_prediction = Prediction.query.filter_by(method_id=9).filter_by(clause_id = entry.id).first()
                    true_prediction.predictedState = counter
                else:
                    new_prediction = Prediction(predictedState=counter, clause_id=entry.id, method_id=9, agb_id=agbid)
                    db.session.add(new_prediction)
                    db.session.commit()
            elif type == "paragraph":
                entry = Paragraph.query.get(item['id'])
                if agb.paragraphIsLabeled == True:
                    true_prediction = Prediction.query.filter_by(method_id=9).filter_by(paragraph_id = entry.id).first()
                    true_prediction.predictedState = counter
                else:
                    new_prediction = Prediction(predictedState=counter, paragraph_id=entry.id, method_id=9, agb_id=agbid)
                    db.session.add(new_prediction)
                    db.session.commit()
